...
CommonJS works too — `const { CodeGraph } = require('@thomsen/codegraph');`.
...
import {
  ...
  FileLock,
} from '@thomsen/codegraph';
```

## Embedding requirements

- **Install from npm** (`npm i @thomsen/codegraph`) so the matching per-platform package — which carries the compiled library — is fetched alongside the shim.
