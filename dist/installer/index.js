...
async function runUninstaller(opts) {
    ...
    if (opts.location) {
        ...
        const sel = await clack.select({
            ...
            options: [
                { value: 'global', label: 'All projects (global)', hint: '~/.claude, ~/.cursor, ~/.codex, ~/.config/opencode, ~/.hermes, ~/.gemini, ~/.kiro, ~/.copilot, ~/.config/github-copilot' },
                { value: 'local', label: 'Just this project (local)', hint: './.claude, ./.cursor, ./.vscode, ./opencode.jsonc, ./.gemini, ./.kiro' },
            ],
            initialValue: 'global',
        });
        if (clack.isCancel(sel)) {
