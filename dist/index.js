...
class CodeGraph {
    ...
    sourceRootMutex = new utils_1.Mutex();
    ...
    wireLayers() {
        ...
        const workspace = this.workspaceManifest ? (0, workspace_1.createWorkspaceResolution)(this.workspaceManifest) : null;
        this.resolver = (0, resolution_1.createResolver)(this.projectRoot, this.queries, workspace
            ? {
        ...
        this.contextBuilder = (0, context_1.createContextBuilder)(this.projectRoot, this.queries, this.traverser, workspace?.resolveFilePath);
    }
    /**
    ...
    static async init(projectRoot, options = {}) {
        ...
        if (fs.existsSync(path.join(resolvedRoot, workspace_1.WORKSPACE_MANIFEST_PATH))) {
            instance.workspaceManifest = (0, workspace_1.loadWorkspaceManifest)(resolvedRoot);
            instance.wireLayers();
        }
    ...
    static async initWorkspace(root) {
        const resolvedRoot = path.resolve(root);
        const manifest = (0, workspace_1.loadWorkspaceManifest)(resolvedRoot);
    ...
    resolveFilePath(filePath) {
        if (this.workspaceManifest) {
            return (0, workspace_1.createWorkspaceResolution)(this.workspaceManifest).resolveFilePath(filePath);
        }
        return (0, utils_1.validatePathWithinRoot)(this.projectRoot, filePath);
    }
    // ===========================================================================
    // Indexing
    // ===========================================================================
    ...
    async indexSourceRoot(sourceRoot, pathPrefix, options = {}) {
        const original = this.orchestrator;
        this.orchestrator = new extraction_1.ExtractionOrchestrator(fs.realpathSync(path.resolve(sourceRoot)), this.queries, pathPrefix);
        try {
    ...
    async syncSourceRoot(sourceRoot, pathPrefix, options = {}) {
    ...
    async syncWorkspace(options = {}) {
        ...
        for (const member of this.workspaceManifest.members) {
            ...
            for (const name of result.definitionDelta ?? [])
                definitionDelta.add(name);
        }
        combined.durationMs = Date.now() - startedAt;
        if (changedPaths.size > 0)
            combined.changedFilePaths = [...changedPaths];
        if (definitionDelta.size > 0)
            combined.definitionDelta = [...definitionDelta];
        return combined;
    }
    /**
     ...
     */
    async replaceWorkspaceMember(memberName, nextPath) {
        if (!this.workspaceManifest) {
            throw new Error('CodeGraph was not opened as a workspace');
        }
        ...
        if (memberIndex < 0)
            throw new Error(`Unknown CodeGraph workspace member: ${memberName}`);
        ...
        const candidateManifest = {
            ...current,
            members: current.members.map((member, index) => index === memberIndex ? { name: member.name, path: canonicalNextPath } : member),
        };
        ...
        try {
            (0, workspace_1.writeWorkspaceManifest)(stagingRoot, candidateManifest);
            ...
            await this.sourceRootMutex.withLock(async () => this.indexMutex.withLock(async () => {
                this.fileLock.acquire();
                const liveDbPath = (0, db_1.getDatabasePath)(this.projectRoot);
                const stagedDbPath = (0, db_1.getDatabasePath)(stagingRoot);
                ...
                try {
                    ...
                    (0, workspace_1.writeWorkspaceManifest)(this.projectRoot, candidateManifest);
                    manifestReplaced = true;
                    this.db = db_1.DatabaseConnection.open(liveDbPath);
                    this.queries = new queries_1.QueryBuilder(this.db.getDb());
                    ...
                    catch { /* it may already be closed */ }
                    for (const live of liveFiles)
    ...
    watchWorkspace(options = {}) {
        ...
        if (this.workspaceWatchers.some(({ watcher }) => watcher.isActive()))
            return true;
        this.workspaceWatchers = this.workspaceManifest.members.map((member) => ({
            member,
            watcher: new sync_1.FileWatcher(member.path, async (paths) => {
    ...
    getPendingFiles() {
        ...
        const workspacePending = this.workspaceWatchers.flatMap(({ member, watcher }) => watcher.getPendingFiles().map((pending) => ({
            ...pending,
            path: (0, utils_1.normalizePath)(path.join(member.name, pending.path)),
        })));
