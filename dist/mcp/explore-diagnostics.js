...
exports.ExploreDiagnostics = void 0;
exports.attributeSourceBytes = attributeSourceBytes;
...
function resolveSink() {
    ...
    if (OFF.has(lower))
        return null;
    if (STDERR_TABLE.has(lower))
        return { kind: 'stderr', json: false };
    if (lower === 'json')
        return { kind: 'stderr', json: true };
...
class ExploreDiagnostics {
    ...
    scoreFloor = 0;
    maxGraph = 0;
    graphGateThreshold = 0;
    graphGateApplied = false;
    note = '';
    session;
    allocPool = 0;
    allocCliffAt = 0;
    allocCliffed = [];
    constructor(sink, query, projectRoot, budget, maxFiles, indexedFileCount) {
        this.sink = sink;
        this.query = query;
     ...
     */
    static start(query, projectRoot, budget, maxFiles, indexedFileCount) {
        try {
     ...
     */
    setLowValueFiltered(grouped, kept) {
    ...
    noteSession(prior) {
        ...
        for (const call of [...prior.calls].reverse()) {
            for (const file of call.files) {
                ...
                const spans = file.ranges.map((r) => [r.start, r.end]);
                if (existing) {
     ...
     */
    setAllocation(allowances, cliffed, cliffAt, pool) {
        this.allocPool = pool;
     ...
     */
    recordFunded(path, chars) {
        const rec = this.files.get(path);
        if (rec)
            rec.funded = chars;
    }
    /** A candidate rendered source into the response. */
    recordRender(path, render, sourceChars, clipped) {
        const rec = this.files.get(path);
        if (!rec)
            return;
        rec.render = render;
     ...
     */
    recordDedup(path, savedChars, covered) {
        const rec = this.files.get(path);
        if (!rec)
            return;
        rec.dedupSavedChars = savedChars;
        rec.dedupCovered = covered.map((r) => [r.start, r.end]);
    }
    /**
     ...
     */
    finish(finalText, allocatedChars, hardCeiling, filesIncluded) {
        try {
    ...
    buildReport(finalText, allocatedChars, hardCeiling, filesIncluded) {
        const envelope = finalText.length;
        ...
        return {
            ...
            files: records
                ...
                dedupCovered: r.dedupCovered.map((s) => [...s]),
    ...
    emit(report) {
        try {
            if (this.sink.kind === 'file') {
                (0, fs_1.appendFileSync)(this.sink.path, JSON.stringify(report) + '\n', 'utf-8');
                return;
            }
            if (this.sink.json) {
...
exports.ExploreDiagnostics = ExploreDiagnostics;
 ...
 */
function attributeSourceBytes(finalText) {
    const out = new Map();
    if (!finalText)
        return out;
...
function renderTable(report) {
    ...
    if (report.session) {
        ...
        out.push(`  session call #${s.callIndex} for this project` +
        ...
        if (s.priorFiles.length > 12)
            out.push(`    … +${s.priorFiles.length - 12} more already-served file(s)`);
    }
    out.push(`  envelope ${num(env.chars)} chars delivered · ${num(env.allocatedChars)} allocated` +
    ...
    out.push(`  source ${num(env.sourceChars)} (${pct(env.sourceShare)})` +
        ` · meta ${num(env.metaChars)} (${pct(env.metaShare)})` +
        ` · per-file cap ${num(budget.maxCharsPerFile)}`);
    out.push(`  files ${num(sel.filesGrouped)} grouped` +
    ...
    out.push(`  relevance gate ${sel.graphGateApplied ? 'applied' : 'not applied'}` +
    ...
    if (dedup && dedup.savedChars > 0) {
        out.push(`  dedup ${num(dedup.savedChars)} chars not re-sent` +
    ...
    out.push(`  allocation ${num(alloc.reserved)} reserved of ${num(alloc.pool)} pool` +
    ...
    if (skipped.length > 0) {
        ...
        for (const f of skipped.slice(0, 15)) {
            out.push(`    #${String(f.rank).padStart(2)} ${f.path} — ${f.skipped ?? f.render ?? 'not reached'}` +
        ...
        if (skipped.length > 15)
            out.push(`    … and ${skipped.length - 15} more`);
    }
    return out.join('\n');
}
function flagString(f) {
    ...
    if (f.ambientDeclaration)
        flags.push('ambient-decl');
    return flags.join(' ') || '-';
}
//# sourceMappingURL=explore-diagnostics.js.map