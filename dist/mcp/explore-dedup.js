...
exports.EXPLORE_DEDUP = void 0;
exports.exploreDedupEnabled = exploreDedupEnabled;
exports.fileFingerprint = fileFingerprint;
exports.mergeRanges = mergeRanges;
exports.intersectRange = intersectRange;
exports.subtractRange = subtractRange;
exports.dedupeRange = dedupeRange;
exports.servedRangesForFile = servedRangesForFile;
exports.formatSpans = formatSpans;
exports.formatBackReference = formatBackReference;
exports.symbolsInSpans = symbolsInSpans;
const crypto_1 = require("crypto");
exports.EXPLORE_DEDUP = {
    /**
...
function fileFingerprint(content) {
    return `${content.length}:${(0, crypto_1.createHash)('sha1').update(content).digest('hex').slice(0, 16)}`;
}
...
function intersectRange(range, served) {
    const out = [];
...
function subtractRange(range, cut) {
    const out = [];
    ...
    for (const c of mergeRanges(cut)) {
        ...
        if (c.start > range.end)
            break;
        if (c.start > cursor)
            out.push({ start: cursor, end: Math.min(c.start - 1, range.end) });
 ...
 * pointers.
 */
function dedupeRange(range, served, minCovered = exports.EXPLORE_DEDUP.MIN_COVERED_LINES) {
    if (served.length === 0 || range.end < range.start)
        return { emit: [range], covered: [] };
 ...
 */
function servedRangesForFile(prior, filePath, fingerprint) {
    if (!prior)
        return [];
    const spans = [];
...
function formatSpans(spans) {
    const shown = spans.slice(0, exports.EXPLORE_DEDUP.MAX_SPANS_IN_POINTER)
        .map((r) => (r.start === r.end ? `L${r.start}` : `L${r.start}-${r.end}`))
        .join(', ');
    const more = spans.length - exports.EXPLORE_DEDUP.MAX_SPANS_IN_POINTER;
 ...
 */
function formatBackReference(filePath, covered, symbols, opts) {
    const names = symbols.slice(0, exports.EXPLORE_DEDUP.MAX_SYMBOLS_IN_POINTER);
...
function symbolsInSpans(nodes, spans) {
    const out = [];
    const seen = new Set();
    for (const n of nodes) {
        if (n.kind === 'import' || n.kind === 'export')
            continue;
        if (!spans.some((s) => n.startLine <= s.end && (n.endLine || n.startLine) >= s.start))
            continue;
        if (seen.has(n.name))
            continue;
        seen.add(n.name);
...
//# sourceMappingURL=explore-dedup.js.map