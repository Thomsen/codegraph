...
exports.timestampStderrLines = timestampStderrLines;
exports.stampLogChunk = stampLogChunk;
exports.watchdogProgressPaths = watchdogProgressPaths;
...
function timestampStderrLines() {
    ...
    process.stderr.write = ((chunk, ...rest) => {
        return orig(stampLogChunk(chunk), ...rest);
    });
}
/** Prepend `[<ISO-8601>] ` to a log chunk; unknown chunk types pass through. */
function stampLogChunk(chunk) {
    try {
        const stamp = `[${new Date().toISOString()}] `;
        if (typeof chunk === 'string')
            return stamp + chunk;
        if (Buffer.isBuffer(chunk))
            return Buffer.concat([Buffer.from(stamp), chunk]);
    }
 ...
 */
function watchdogProgressPaths(root) {
    if (!root)
        return {};
    const dbPath = path.join((0, directory_1.getCodeGraphDir)(root), 'codegraph.db');
...
class MCPServer {
    ...
    async startDirect(reason) {
        ...
        this.livenessWatchdog = (0, liveness_watchdog_1.installMainThreadWatchdog)(watchdogProgressPaths(resolveDaemonRoot(this.projectPath)));
    }
    /**
    ...
    async startDaemonProcess() {
        ...
        for (let attempt = 0; attempt < TAKEOVER_MAX_RETRIES; attempt++) {
            ...
            if (lock.kind === 'acquired') {
                ...
                this.livenessWatchdog = (0, liveness_watchdog_1.installMainThreadWatchdog)(watchdogProgressPaths(root));
