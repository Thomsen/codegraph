...
exports.ToolHandler = exports.tools = exports.EXPLORE_ALLOCATION = exports.RELEVANCE_KIND_WEIGHT = exports.PathRefusalError = exports.NotIndexedError = void 0;
exports.__setLoadCodeGraphForTests = __setLoadCodeGraphForTests;
...
exports.allocateExploreBudget = allocateExploreBudget;
...
const loadCodeGraph = () => loadCodeGraphForTests ?? require('../index').default;
...
let loadCodeGraphForTests = null;
function __setLoadCodeGraphForTests(cls) {
...
const explore_diagnostics_1 = require("./explore-diagnostics");
 ...
 */
const WEAK_RELEVANCE_KINDS = new Set([
 ...
 */
const RELEVANCE_USAGE_EDGES = new Set([
 ...
 * every tier.
 */
function allocateExploreBudget(candidates, budget, maxFiles) {
    const A = exports.EXPLORE_ALLOCATION;
    const empty = { allowances: new Map(), cliffed: [], cliffAt: 0, pool: 0 };
    ...
    for (const c of candidates) {
        if (!c.spine && (weights.get(c.path) ?? 0) < cliffAt)
            cliffed.push(c.path);
        else
            admitted.push(c);
    }
    ...
    for (const c of admitted.slice(maxFiles))
        cliffed.push(c.path);
 ...
 */
function pointerLineFor(filePath, nodes) {
...
class ToolHandler {
    ...
    withSessionView(toolName, args, sessionState) {
        if (!(explore_session_state_1.EXPLORE_SESSION_VIEW_ARG in args) && (!sessionState || toolName !== 'codegraph_explore')) {
            return args;
        }
        const copy = { ...args };
    ...
    buildFlowFromNamedSymbols(cg, query) {
        ...
        try {
            ...
            const identityOnly = () => (preciseNamedIds.size === 0 ? EMPTY : {
                text: '',
                pathNodeIds: new Set(),
                namedNodeIds: new Set(preciseNamedIds),
                uniqueNamedNodeIds: new Set([...uniqueNamedNodeIds].filter((id) => preciseNamedIds.has(id))),
                spineCallSites: new Map(),
            });
            if (named.size < 2) {
    ...
    async handleExplore(args) {
        ...
        const diag = explore_diagnostics_1.ExploreDiagnostics.start(query, projectRoot, budget, maxFiles, indexedFileCount);
        ...
        const subgraphUsageDegree = new Map();
        for (const edge of subgraph.edges) {
            if (!RELEVANCE_USAGE_EDGES.has(edge.kind) || edge.source === edge.target)
                continue;
        ...
        const isUsageIsolated = (node) => {
            if ((subgraphUsageDegree.get(node.id) ?? 0) > 0)
                return false;
            ...
            let isolated = true;
            try {
                const used = (e) => RELEVANCE_USAGE_EDGES.has(e.kind);
        ...
        const relevanceWeight = (node, probeIsolation) => {
            const weight = exports.RELEVANCE_KIND_WEIGHT[node.kind] ?? DEFAULT_RELEVANCE_KIND_WEIGHT;
            if (!probeIsolation || !WEAK_RELEVANCE_KINDS.has(node.kind))
                return weight;
        ...
        const isLowValue = (p) => {
            ...
            return (/(?:^|\/)(tests?|__tests?__|specs?)\//.test(lp) ||
                /_test\.go$/.test(lp) ||
                /(?:^|\/)test_[^/]+\.py$/.test(lp) ||
                /_test\.py$/.test(lp) ||
                /_spec\.rb$/.test(lp) ||
        ...
        const isDampedDeclaration = (filePath) => isAmbientDeclaration(filePath) && !namedTypeFiles.has(filePath);
        /**
         ...
         */
        const rankPenalty = (filePath) => Math.min(isGeneratedCandidate(filePath) ? GENERATED_RANK_PENALTY : 1, isDampedDeclaration(filePath) ? AMBIENT_DECLARATION_RANK_PENALTY : 1)
        ...
        const scoreFloor = Math.max(SCORE_FLOOR_ABSOLUTE, Math.min(SCORE_FLOOR_MAX, topScore * SCORE_FLOOR_FRACTION_OF_TOP));
        ...
        for (const [fp, mass] of fileGraphScore)
            fileGraphScore.set(fp, mass * rankPenalty(fp));
        ...
        for (const t of changeSurfaceCandidates) {
            ...
            if (!group) {
                group = { nodes: [], score: 0, peripheral: 0 };
        ...
        const allocation = allocateExploreBudget(sortedFiles.map(([fp, group]) => ({
            path: fp,
            score: group.score,
        ...
        const sourceCeiling = reservedTotal + Math.round(budget.maxOutputChars * exports.EXPLORE_ALLOCATION.WHOLE_FILE_BUY_OVERSHOOT_FRACTION);
        /**
        ...
        const sectionOverhead = (filePath, nodes) => {
            const hit = overheadCache.get(filePath);
            if (hit !== undefined)
                return hit;
            const names = [...new Set(nodes.filter((n) => n.kind !== 'import' && n.kind !== 'export')
                    .map((n) => `${n.name}(${n.kind})`))].slice(0, budget.maxSymbolsInFileHeader);
         ...
         */
        const owedPayableBelow = (fileIndex, budgetLeft) => {
            let held = 0;
            for (let j = fileIndex + 1; j < sortedFiles.length; j++) {
                ...
                if (held + need > budgetLeft) {
                    ...
                    if (partial >= exports.EXPLORE_ALLOCATION.MIN_CHARS + overhead)
                        held += partial;
                    break;
                }
                held += need;
        ...
        for (let fileIndex = 0; fileIndex < sortedFiles.length; fileIndex++) {
            ...
            // reservation time.
            const allowance = Math.min(reserved + Math.max(0, reservedSoFar - sourceSpent), Math.max(reserved, Math.round(budget.maxOutputChars * exports.EXPLORE_ALLOCATION.MAX_SHARE)));
            ...
            const fundedHeadroom = Math.max(Math.min(reserved, headroom), headroom - owedPayableBelow(fileIndex, Math.max(0, headroom - reserved)));
             ...
             */
            const dedupeSpans = (parts) => {
                if (served.length === 0)
                    return { parts: [...parts], covered: [] };
                const kept = [];
                const covered = [];
                for (const part of parts) {
                    const split = (0, explore_dedup_1.dedupeRange)(part.range, served);
                    ...
                    for (const r of split.emit)
                        kept.push({ range: r, text: renderSpan(r) });
                }
                return { parts: kept, covered: (0, explore_dedup_1.mergeRanges)(covered) };
            };
            const coveredChars = (spans) => spans.reduce((sum, r) => sum + fileLines.slice(r.start - 1, r.end).join('\n').length, 0);
            /**
            ...
            if (!fileStale && adaptiveExploreEnabled() && flow.pathNodeIds.size > 0
                ...
                if (skel.length > 0) {
                    ...
                    const withTail = (parts) => [...parts.map((p) => p.text), ...(sigTail ? [sigTail] : [])].join('\n');
                    emitFileSection({
            ...
            const graceBound = allowance + Math.min(exports.EXPLORE_ALLOCATION.WHOLE_FILE_GRACE_MAX, Math.round(allowance * exports.EXPLORE_ALLOCATION.WHOLE_FILE_GRACE_FRACTION));
            ...
            const buysWhole = fileContent.length <= graceBound
                || (reserved >= fileContent.length * exports.EXPLORE_ALLOCATION.WHOLE_FILE_BUY_FRACTION
            ...
            if (fileLines.length <= WHOLE_FILE_MAX_LINES && buysWhole) {
                ...
                const wholeRange = { start: 1, end: body.split('\n').length };
            ...
            const sectionText = (parts) => parts.map((p) => p.text).join(GAP_MARKER);
             ...
             */
            const shrinkCluster = (c, cap) => {
                if (c.members.length < 2)
                    return null;
                const byImportance = [...c.members].sort((a, b) => b.importance - a.importance || (a.end - a.start) - (b.end - b.start) || a.start - b.start);
                const sizeOf = (r) => fileLines.slice(r.start - 1, r.end).join('\n').length;
                const keep = [];
                let kept = 0;
                ...
                if (keep.length === c.members.length)
                    return null;
                ...
                for (const r of keep) {
                    ...
                    if (last && r.start <= last.end + gapThreshold)
            ...
            const lineCost = (ln) => (fileLines[ln - 1] ?? '').length + 1 + (withLineNumbers ? String(ln).length + 1 : 0);
            /**
             ...
             */
            const headWindowOf = (r, room, minLines = 0) => {
                let end = r.start - 1;
                let chars = 0;
                for (let ln = r.start; ln <= r.end; ln++) {
                    const cost = lineCost(ln);
                    if (chars + cost > room && end - r.start + 1 >= minLines)
                        break;
                    chars += cost;
                    end = ln;
                }
            ...
            const centeredWindowOf = (line, lo, hi, room) => {
                if (line < lo || line > hi)
                    return null;
                let start = line, end = line, chars = lineCost(line);
                for (let grown = true; grown;) {
                    grown = false;
                    if (end + 1 <= hi && chars + lineCost(end + 1) <= room) {
                        end += 1;
                        chars += lineCost(end);
                        grown = true;
                    }
                    if (start - 1 >= lo && chars + lineCost(start - 1) <= room) {
                        start -= 1;
             ...
             */
            const windowToCeiling = (parts, ceiling, focusLines = []) => {
                const inParts = (line) => parts.some((p) => line >= p.range.start && line <= p.range.end);
                ...
                const fill = (room) => {
                    ...
                    for (const p of parts) {
                        ...
                        const win = headWindowOf(p.range, Math.max(0, room - used - join), first ? MIN_WINDOW_LINES : 0);
                ...
                for (let i = 0; i < pending.length; i++) {
                    ...
                    if (line <= covered)
                        continue; // an earlier window already reached it
                    ...
                    if (!win || win.end - win.start + 1 < MIN_WINDOW_LINES)
                        continue;
                    emit.push(win);
            ...
            ceiling = Infinity) => {
                ...
                const bound = (r) => {
                    if (!Number.isFinite(ceiling) || sectionText(r.parts).length <= ceiling)
                        return r;
            ...
            const renderedClusters = new Map();
            ...
            for (const rc of rankedClusters) {
                ...
                if (room < exports.EXPLORE_ALLOCATION.MIN_CHARS)
                    continue;
                ...
                if (projectedChars + text.length + GAP_MARKER.length > cap)
                    continue;
                renderedClusters.set(rc.idx, section);
                anyClusterShrunk = anyClusterShrunk || section.shrunk;
                chosenIndices.add(rc.idx);
            ...
            const costOfSection = (header, body) => header.length + 2 + (body.length > 0 ? body.length + lang.length + 11 : 0);
            ...
            while (totalChars + costOfSection(fileHeader, assembled.text) > renderCeiling
                ...
                if (room >= exports.EXPLORE_ALLOCATION.MIN_CHARS && !reshrunkOnce.has(weakest)) {
            ...
            emitFileSection({
                ...
                covered: (0, explore_dedup_1.mergeRanges)(coveredRanges),
                overhead: 200,
                mode: 'clusters',
        ...
        const restore = suppressedFallback;
        if (newSourceChars === 0 && restore) {
            if (totalChars + restore.sourceChars + restore.overhead <= renderCeiling) {
                ...
                if (idx >= 0)
                    backReferencedFiles.splice(idx, 1);
        ...
        const completenessBlock = budget.includeCompletenessSignal
        ...
        // document order.
        const roomFor = (block) => block.reduce((n, s) => n + s.length + 1, 0);
        ...
        if (pointerEntries.length > 0) {
            ...
            if (left >= 0) {
                ...
                if (taken > 0) {
                    ...
                    if (dropped > 0)
                        pointerBlock.push(`- ... and ${dropped} more files`);
