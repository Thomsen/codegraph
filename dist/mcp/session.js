...
class MCPSession {
    ...
    async handleInitialize(request) {
        ...
        try {
            explicitPath = (0, workspace_1.discoverCodeGraphRoot)({
            ...
            if (error instanceof workspace_1.WorkspaceRootAmbiguityError) {
        ...
        this.transport.sendResult(request.id, {
            ...
            workspaceProtocolVersion: workspace_1.WORKSPACE_PROTOCOL_VERSION,
            capabilities: { tools: {} },
    ...
    async handleToolsList(request) {
        try {
            ...
            this.transport.sendError(request.id, transport_1.ErrorCodes.InvalidParams, error instanceof Error ? error.message : String(error));
            return;
        }
    ...
    async retryInitIfNeeded() {
        if (this.rootDiscoveryError)
            throw this.rootDiscoveryError;
    ...
    async initFromRoots() {
        ...
        try {
            ...
            const discovered = (0, workspace_1.discoverCodeGraphRoot)({ advertisedRoots: rootPaths(result), cwd: process.cwd() });
            if (discovered) {
                target = discovered;
            }
            else {
            ...
            if (err instanceof workspace_1.WorkspaceRootAmbiguityError) {
