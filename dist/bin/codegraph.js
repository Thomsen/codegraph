...
function main() {
    ...
    workspace
        ...
        if (options.json)
            ...
            console.log(`CodeGraph workspace protocol version ${output.protocolVersion}`);
    });
    workspace
        .command('init')
        ...
        try {
            ...
            if (options.json)
                ...
                success(`Initialized CodeGraph workspace "${manifest.workset}" with ${manifest.members.length} member(s).`);
        }
        catch (err) {
    ...
    workspace
        ...
        try {
            ...
            if (options.json)
                ...
                console.log(`CodeGraph workspace "${manifest.workset}": ${initialized ? 'initialized' : 'not initialized'}`);
        }
        catch (err) {
            const message = err instanceof Error ? err.message : String(err);
            if (options.json)
    ...
    workspace
        ...
        .requiredOption('--root <path>', 'Workspace root containing .codegraph/workspace.json')
        .option('--json', 'Output machine-readable JSON')
        .action(async (member, nextPath, options) => {
        const root = fs.realpathSync(path.resolve(options.root));
        try {
            ...
            if (options.json)
                ...
                success(`Replaced CodeGraph workspace member "${member}" with ${replaced?.path}.`);
        }
        catch (err) {
            const message = err instanceof Error ? err.message : String(err);
            if (options.json)
