 ...
 * safely down-weight.
 */
function deriveProjectNameTokens(projectRoot) {
 ...
 */
function scorePathRelevance(filePath, query, projectNameTokens) {
    ...
    const words = projectNameTokens && projectNameTokens.size > 0
    ...
    for (const word of scored) {
        ...
        if (subtokens.some((t) => fileName.includes(t)))
            score += 10;
        // Directory match
        if (subtokens.some((t) => dirName.includes(t)))
            score += 5;
        // General path match
        else if (subtokens.some((t) => pathLower.includes(t)))
            score += 3;
    }
