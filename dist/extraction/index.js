...
class ExtractionOrchestrator {
    ...
    constructor(rootDir, queries, pathPrefix = '') {
        this.rootDir = rootDir;
        this.queries = queries;
        this.pathPrefix = pathPrefix;
    }
    indexedPath(sourcePath) {
        return this.pathPrefix ? (0, utils_1.normalizePath)(path.join(this.pathPrefix, sourcePath)) : sourcePath;
    }
    sourcePath(indexedPath) {
        if (!this.pathPrefix)
            return indexedPath;
        const prefix = `${(0, utils_1.normalizePath)(this.pathPrefix)}/`;
    ...
    storeWriterOpts) {
         ...
         */
        const parseFile = (sourcePath, content) => {
            const filePath = this.indexedPath(sourcePath);
            const language = (0, grammars_1.detectLanguage)(sourcePath, content, overrides);
            if (!pool)
        ...
        const storeResult = async (sourcePath, content, stats, result) => {
            const filePath = this.indexedPath(sourcePath);
            processed++;
        ...
        const recordParseFailure = (sourcePath, err) => {
            const filePath = this.indexedPath(sourcePath);
            processed++;
            filesErrored++;
            errors.push({
        ...
        if (retryableErrors.length > 0 && pool) {
            ...
            for (const errEntry of retryableErrors) {
                ...
                let content;
                try {
                    const fullPath = (0, utils_1.validatePathWithinRoot)(this.rootDir, sourcePath);
                    if (!fullPath)
                        continue;
            ...
            if (stillFailing.length > 0) {
                ...
                for (const errEntry of stillFailing) {
                    ...
                    if (result.nodes.length > 0 || result.errors.length === 0) {
                        const language = (0, grammars_1.detectLanguage)(sourcePath, fullContent, overrides);
                        const stats = await fsp.stat(path.join(this.rootDir, sourcePath));
    ...
    async indexFileWithContent(relativePath, content, stats) {
        ...
        const result = (0, tree_sitter_1.extractFromSource)(indexedPath, content, language, frameworkNames);
        // Store in database
        if (result.nodes.length > 0 || result.errors.length === 0) {
            await this.storeExtractionResult(indexedPath, content, language, stats, result, (0, cooperative_yield_1.createYielder)());
        }
        return result;
    }
    /**
    ...
    scopedPaths) {
        ...
        if (filesToIndex.length > 0) {
            for (const pair of this.queries.getNodeNamePairsByFiles(filesToIndex.map((file) => this.indexedPath(file))))
                pairsBefore.add(pair);
        }
